...
describe 'More Merchant Stats - Fulfillment Speed by City/State' do
  ...
  context 'as a logged in user' do
    it 'shows top_5 merchants for state and city' do
      ...
      within ".top_5_stats" do
        within '.top_5_state' do
          expect(all('.top_merchant_state')[0]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[0]}).last}, Fulfillment Time: #{@top_5_state[0].fulfill_speed}")
          expect(all('.top_merchant_state')[1]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[1]}).last}, Fulfillment Time: #{@top_5_state[1].fulfill_speed}")
          expect(all('.top_merchant_state')[2]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[2]}).last}, Fulfillment Time: #{@top_5_state[2].fulfill_speed}")
          expect(all('.top_merchant_state')[3]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[3]}).last}, Fulfillment Time: #{@top_5_state[3].fulfill_speed}")
          expect(all('.top_merchant_state')[4]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[4]}).last}, Fulfillment Time: #{@top_5_state[4].fulfill_speed}")
        end

        top_5_merchant_ids = @top_5_city.map {|m| m.merchant_id}
        top_merch = User.get_merchant_info(top_5_merchant_ids)

        within 'top_5_city' do
          expect(all('.top_merchant_city')[0]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[0]}).last}, Fulfillment Time: #{@top_5_city[0].fulfill_speed}")
          expect(all('.top_merchant_city')[1]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[1]}).last}, Fulfillment Time: #{@top_5_city[1].fulfill_speed}")
          expect(all('.top_merchant_city')[2]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[2]}).last}, Fulfillment Time: #{@top_5_city[2].fulfill_speed}")
          expect(all('.top_merchant_city')[3]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[3]}).last}, Fulfillment Time: #{@top_5_city[3].fulfill_speed}")
          expect(all('.top_merchant_city')[4]).to have_content("Merchant: #{(top_merch.select{|m| m.first = top_5_merchant_ids[4]}).last}, Fulfillment Time: #{@top_5_city[4].fulfill_speed}")
        end
      end
    end
  end
